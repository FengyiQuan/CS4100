    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i + 1]]
        "*** YOUR CODE HERE ***"
        max_belief = [max(dist, key=lambda x: dist[x]) for dist in livingGhostPositionDistributions]
        ...
        for action in legal:
            val = min(
                self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), ghost) for ghost in max_belief)
            if min_dis > val:
